...
public class UserMenu {
	...
	public void userSearch() {	
		while(true) {
			System.out.println("\n[도서검색 및 목록]");
			String search = conInput("검색할 도서명을 입력하세요");
		...
		while(true) {
			...
			if(select.equals("1")) {
				...
				if (!(vo.getBookName() == null)) {
					...
					while(true){
						String bName = conInput("\n대여하실 도서명을 입력헤주세요. (Enter 입력 시 대여가능한 도서목록 확인 가능합니다.)");
						Set<String> keySet = DataSet.bookData.keySet();
						Iterator<String> i = keySet.iterator();
						try {	
							...
							while(i.hasNext()) {
								...
								if(vo2.getBookName().indexOf(bName) >= 0) {
									if(count==0) {
										System.out.println("도서명\t\t\t저자\t\t\t장르\t\t\t출판사\t\t\t재고량");
									}
									if(vo2.getRentDay() == null) {
										System.out.printf("%-15s%-15s%-15s%-15s%-15s\n", 
											vo2.getBookName(), vo2.getBookAuthor(), vo2.getBookGenre(), vo2.getPublisher(), vo2.getStock());
									}
									count ++;
								}//end if
							}//end while
		...
		String change = conInput("[1.개인정보 수정]  [2. 회원탈퇴]  [3.메뉴 돌아가기] ");
			if(change.equals("1")) {
				while(true) {
					...
					if(select.equals("1")) {
						String tel = conInput("수정하실 연락처를 입력해 주세요 ");
						vo.setUserTel(tel);
						System.out.println("연락처 수정이 완료되었습니다.\n");
						break;
					}else if(select.equals("2")) {
						String pwd = conInput("수정하실 비밀번호를 입력해 주새요 ");
						vo.setUserPwd(pwd);
